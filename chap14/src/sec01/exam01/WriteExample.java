...
// Java에서는 스트림을 통해 파일, 네읕워크, 키보드, 메모리 등에서 데이터를 주고받을 수 있음
...
public class WriteExample {
	public static void main(String[] args) throws IOException {
		...
		os.flush(); // 버퍼를 비움(버퍼에 쌓여있는 데이터를 한꺼번에 출력스트림으로 보냄)
		
		// 참고: 모든 출력 스트림이 다 버퍼를 갖고 있진 않다.
		// FileoutputStream에는 내부 버퍼가 존재하지 않음
		// 즉, write() 시 곹바로 OS에 전달되어 출력
		...
		// close()를 명시적으로 호출하지 않아도, try 블록이 끝나거나 예외가 발생되는 즉시 자동으로 자원정리
		// AutoCloseable을 구현한 클래스는 try-with-rewources로 자동 종료 가능
