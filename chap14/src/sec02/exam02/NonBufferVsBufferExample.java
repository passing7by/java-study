...
public class NonBufferVsBufferExample {
	public static void main(String[] args) throws Exception {
		// 참고: 클래스명.class란? 해당 클래스의 Class 객체를 가리키느 문법
		...
		bis.close();
		bos.close();
		// 보조 스트림만 close() 해도 내부의 기본 스트림가지 자동으로 닫힘
		// try-with-resourdes를 사용하는 것이 가장 안전하고 깔끔
		try (
	...
	static int data = -1; // 입력 스트림으로부터 데이터를 전현 읽지 않앗을 경우 -1
	// 매개 변수의 타입은 바이트 기반 스트림으로 -> 그림, 음악, 테스트 등 모든 종류의 파일을 복사 하기 위해
