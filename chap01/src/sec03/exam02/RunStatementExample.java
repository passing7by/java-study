 ...
 * 
 * - 실행 및 디버깅 Ctrl + F11 : 현재 파일 실행 F11 : 현재 파일 디버깅 실행 Ctrl + Shift + B : 현재 줄에
 * 브레이크포인트 설정/해제 F6 : 디버깅 중 다음 줄까지 실행 (Step Over) F5 : 메서드 내부 진입 (Step Into) F7
 * : 현재 메서드에서 빠져나옴 (Step Return)
 * 
 * - 프로젝트 관리 및 리팩토링 Ctrl + n + java project : 새 프로젝트 생성 Ctrl + n + java class :
 * 새 클래스 생성 Alt + Shift + N → C : 새 클래스 생성 Alt + Shift + S : 소스 메뉴 Alt + Shift +
 * T : 리팩토링 메뉴 (Extract Method 등)
 * 
 * - 주석 달기 Ctrl + / : 한줄 주석 처리 또는 제거 Ctrl + Shift + / : 블록 주석 Ctrl + Shift + \ :
 * 블록 주석 제거
 * 
 * - Git Ctrl + Shift + 3 : Git Staging
 */
