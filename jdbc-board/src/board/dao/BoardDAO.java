	...
	public List<Board> list() {
		...
		try {
			...
			while (rs.next()) { // 조회된 결과의 다음 행으로 이동
				...
				board.setRegDate(rs.getObject("reg_date", LocalDateTime.class)); // getObject를 이용할 수도 있고
//				board.setUpdDate(rs.getTimestamp("upd_date").toLocalDateTime()); // getTimestamp를 이용할 수도 있음 (getObject가 더 깔끔)
				board.setUpdDate(rs.getObject("upd_date", LocalDateTime.class)); // getObject를 이용할 수도 있고
